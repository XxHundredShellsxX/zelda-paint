...
rotr=draw.rect(screen,(0,0,0),(955,410,40,40))
rotl=draw.rect(screen,(0,0,0),(955,460,40,40))
#default size
S=4
...
while running:
    for evnt in event.get():
        ...
        scanvas=screen.subsurface(RECT).copy()
...
#------------------------------------------------------------------------------#
        ...
        if rotr.collidepoint(mx,my) and mb[0]==1:
            if tool!="crop":
                rotate=transform.rotate(scanvas,90)
                screen.blit(rotate,(400,100))
            else:
                if len(lcrop)>0 and go==False:
                    cimage=transform.rotate(cimage,90)
                    draw.rect(screen,(255,255,255),(d,f,c-d,e-f))
                    screen.blit(cimage,(d,f))

        if rotl.collidepoint(mx,my) and mb[0]==1:
            if tool!="crop":
                rotate=transform.rotate(scanvas,270)
                screen.blit(rotate,(400,100))
            else:
                if len(lcrop)>0 and go==False:
                    cimage=transform.rotate(cimage,270)
                    screen.blit(foldscreen,(0,0))
                    screen.blit(cimage,(d,f))
